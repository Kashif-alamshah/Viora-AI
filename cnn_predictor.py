...
from nn_models import Skin_cnn

BASE_DIR   = os.path.dirname(os.path.abspath(__file__))
MODEL_PATH = os.path.join(BASE_DIR, "Models","best_skin_cnn.pth")
SAVE_DIR   = os.path.join(BASE_DIR, "gradcam_outputs")
os.makedirs(SAVE_DIR, exist_ok=True)

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

model = Skin_cnn(conv1_out=128, conv2_out=128, conv3_out=128,
                 dropout=0.4170098949220182, neurons=128)
...
def _generate_gradcam(img_tensor):
    ...
    # Last Conv2d is feature[-4] (before BN, ReLU, Pool of 3rd block)
...
def cnn_predict(image_path: str) -> dict:
    ...
    plt.suptitle("Skin Lesion Analysis — Custom CNN", fontsize=14, fontweight="bold")
    plt.tight_layout()

    base_name = os.path.splitext(os.path.basename(image_path))[0]
    save_path = os.path.join(SAVE_DIR, f"{base_name}_cnn_{pred_class}.png")
